...
class PatternBarChart(Plot):
    """
    Bar chart showing frequency of row-level missingness patterns for 3 columns.

    Each bar represents a unique combination of missing variables among 3 columns:
    3 single-column missing patterns, 3 two-column patterns, and 1 three-column pattern.
    """
    ...
    def _prepare_columns(self) -> List[str]:
        ...
        if self.selected_columns:
            ...
            if len(cols) < 3:
                raise ValueError("pattern_barchart requires at least 3 valid columns.")
            return cols[:3]

        missing_rate = self.data.column_missing_rate
        cols = missing_rate.sort_values(ascending=False).head(3).index.tolist()
        if len(cols) < 3:
            raise ValueError("pattern_barchart requires at least 3 columns.")
        return cols

    def _build_figure(self) -> go.Figure:
        cols = self._prepare_columns()
        df = self.data.data
        mask = self.data.missing_mask[cols]
        ...
        # If truncation caused duplicates, append a simple " etc" suffix and
        # keep labels unique without visible numbering.
        ...
        # Add headroom above the tallest bar so labels don't clash.
