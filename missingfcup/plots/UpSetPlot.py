...
class UpSetPlot(Plot):
    """
    UpSet-style visualization for missingness intersections.

    Shows:
    - Set sizes (missing count per column)
    - Intersection sizes (missingness patterns across columns)
    - Matrix of dots indicating which columns are missing per intersection
    """
    ...
    def _prepare_columns(self) -> List[str]:
        ...
        if not cols:
            raise ValueError("UpSet plot requires at least one column with missing values.")
    ...
    def _compute_intersections(self, cols: List[str]):
        ...
        # Drop rows with no missingness among selected columns
        if () in counts.index:
            counts = counts.drop(())
    ...
    def _build_figure(self) -> go.Figure:
        ...
        set_sizes = self.data.column_missing_count.loc[cols]
        ...
        # Align x ranges for intersection bars and matrix
