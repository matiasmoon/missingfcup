...
class _MissingDataColumnsMixin:
    ...
    def col_missing_rate(self) -> pd.Series:
        """
        Proportion of missing values per column (0.0 to 1.0).
    ...
    def col_missing_percent(self) -> pd.Series:
        """Percentage of missing values per column (0 to 100)."""
        return self.col_missing_rate * 100

    @cached_property
    def col_completeness(self) -> pd.Series:
        """
        Proportion of present (non-missing) values per column (0.0 to 1.0).
